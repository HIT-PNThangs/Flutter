...
import 'package:call_api_air_live/app/data/util/app_validation.dart';
...
class NativeBridge {
  ...
  static const platformFramework =
      MethodChannel('com.vietapps.airlive/framework');

  static final String tag =
      '--NativeBridge ${Platform.operatingSystem}--: ';

  Future<NativeResponseModel<String>> nativeEditPhoto(
      String inPath, String outPath) async {
    try {
      final String result = await platformFramework.invokeMethod(
          'editPhoto', <String, dynamic>{'inPath': inPath, 'outPath': outPath});
  ...
  Future<NativeResponseModel<String>> nativeCheckIfWatchAvailable() async {
    try {
      final String result = await platformFramework
          .invokeMethod('checkIfWatchAvailable', <String, dynamic>{});
  ...
  Future<NativeResponseModel<String>> nativeSendImageToWatch(
      String path) async {
    try {
      final String result = await platformFramework
          .invokeMethod('sendImageToWatch', <String, dynamic>{'path': path});
