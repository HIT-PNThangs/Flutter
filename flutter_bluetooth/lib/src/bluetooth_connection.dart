...
class BluetoothConnection {
  ...
  BluetoothConnection._consumeConnectionID(int? id)
      : _id = id,
        _readChannel = EventChannel('${FlutterBluetooth.namespace}/read/$id') {
  ...
  static Future<BluetoothConnection> toAddress(String? address) async {
    ...
    return BluetoothConnection._consumeConnectionID(
        await FlutterBluetooth.methodChannel.invokeMethod('connect', {"address": address}));
  }
...
class _BluetoothStreamSink<Uint8List> extends StreamSink<Uint8List> {
  ...
  void add(Uint8List data) {
    ...
    _chainedFutures = _chainedFutures.then((_) async {
      ...
      await FlutterBluetooth.methodChannel.invokeMethod('write', {'id': _id, 'bytes': data});
    }).catchError((e) {
      exception = e;
      close();
    });
  ...
  Future get allSent => Future(() async {
        ...
        _chainedFutures = Future.value(); // Just in case if Dart VM is retarded
      });
}
