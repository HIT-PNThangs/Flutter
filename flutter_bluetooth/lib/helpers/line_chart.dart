  ...
  Widget build(BuildContext context) {
    return ConstrainedBox(
        ...
        child: CustomPaint(
          ...
          horizontalLabelsTextStyle: horizontalLabelsTextStyle ?? Theme.of(context).textTheme.caption,
          verticalLabelsTextStyle: verticalLabelsTextStyle ?? Theme.of(context).textTheme.caption,
...
class _LineChartPainter extends CustomPainter {
  ...
  _LineChartPainter({
    ...
    required this.seriesLinesPaints,
            ...
            (horizontalLabelsTextStyle?.fontSize ?? 12) + additionalMinimalHorizontalLabelsInterval {
    ...
    if (argumentsLabels != null) {
      ...
      while (entry.moveNext()) {
        ...
        final double goodRatio =
            ((lastWidth + nextWidth) / 2 + additionalMinimalVerticalLablesInterval) / (nextValue - lastValue);
...

    {
      ...
      for (LabelEntry tuple in labels) {
        ...
        final TextPainter textPainter =
            TextPainter(text: TextSpan(text: tuple.label, style: verticalLabelsTextStyle), textDirection: TextDirection.ltr)
              ..layout();
        textPainter.paint(
            canvas, Offset(xOffset - textPainter.width / 2, size.height - (verticalLabelsTextStyle?.fontSize ?? 12) - 8));
      }
    }
