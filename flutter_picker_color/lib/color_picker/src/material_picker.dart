...
class _MaterialPickerState extends State<MaterialPicker> {
  ...
  List<Map<Color, String>> _shadingTypes(List<Color> colors) {
    ...
    for (Color colorType in colors) {
      if (colorType == Colors.grey) {
        ...
        result.addAll(
            [100, 200, 400, 700].map((int shade) => {colorType[shade]!: 'A$shade'}).toList());
  ...
  Widget build(BuildContext context) {
    bool _isPortrait =
        MediaQuery.of(context).orientation == Orientation.portrait || widget.portraitOnly;
    ...
    Widget _colorList() {
      return Container(
        ...
        child: Container(
          margin:
              _isPortrait ? const EdgeInsets.only(right: 10) : const EdgeInsets.only(bottom: 10),
