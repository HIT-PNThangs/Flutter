...
class HlsPlaylistParser {
  ...
  HlsMasterPlaylist _parseMasterPlaylist(Iterator<String> extraLines, String baseUri) {
    ...
    while (extraLines.moveNext()) {
      ...
      if (line.startsWith(tagDefine)) {
        final String? key = _parseStringAttr(source: line, pattern: regexpName, variableDefinitions: variableDefinitions);
        final String? val = _parseStringAttr(source: line, pattern: regexpValue, variableDefinitions: variableDefinitions);
        if (key == null) {
        ...
        final String referenceUri = _parseStringAttr(source: extraLines.current, variableDefinitions: variableDefinitions)!;
    ...
    for (int i = 0; i < variants.length; i++) {
      ...
      if (urlsInDeduplicatedVariants.add(variant.url)) {
        ...
        final HlsTrackMetadataEntry hlsMetadataEntry = HlsTrackMetadataEntry(variantInfos: urlToVariantInfos[variant.url]);
    ...
    mediaTags.forEach((line) {
      ...
      final String? name = _parseStringAttr(source: line, pattern: regexpName, variableDefinitions: variableDefinitions);
      ...
      Format format;
      final HlsTrackMetadataEntry entry = HlsTrackMetadataEntry(groupId: groupId, name: name, variantInfos: <VariantInfo>[]);
  ...
  static SchemeData? _parseDrmSchemeData({String? line, String? keyFormat, Map<String?, String?>? variableDefinitions}) {
    ...
    if (keyFormatWidevinePsshBinary == keyFormat) {
      final String uriString = _parseStringAttr(source: line, pattern: regexpUri, variableDefinitions: variableDefinitions)!;
  ...
  static HlsMediaPlaylist _parseMediaPlaylist(HlsMasterPlaylist masterPlaylist, List<String> extraLines, String baseUri) {
    ...
    for (final line in extraLines) {
      ...
      if (line.startsWith(tagPlaylistType)) {
        ...
        final String? uri = _parseStringAttr(source: line, pattern: regexpUri, variableDefinitions: variableDefinitions);
