...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () =>
            Get.to(() => AddTaskScreen(), transition: Transition.downToUp, duration: Duration(milliseconds: 500)),
      ),
    );
  }
  ...
  Widget _dateBar() {
    return Container(
      ...
      child: DatePicker(
        ...
        dayTextStyle: TextStyle(color: Get.isDarkMode ? Colors.white : darkGreyClr),
        dateTextStyle:
            TextStyle(color: Get.isDarkMode ? Colors.white : darkGreyClr, fontSize: 20, fontWeight: FontWeight.bold),
        monthTextStyle: TextStyle(color: Get.isDarkMode ? Colors.white : darkGreyClr),
      ),
    );
  }
          ...
          child: ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                if (task.repeat == 'Daily' ||
                    ...
                    (task.repeat == 'Weekly' &&
                        _selectedate.difference(DateFormat.yMd().parse(task.date!)).inDays % 7 == 0) ||
  ...
  handlingReminder(int reminder, var myTime, Task task) {
    ...
    if (reminder == 5) {
      notifyHelper.scheduledNotification(minutes < 5 ? hours - 1 : hours, minutes < 5 ? minutes + 55 : minutes - 5, task);
    } else if (reminder == 10) {
      notifyHelper.scheduledNotification(minutes < 10 ? hours - 1 : hours, minutes < 10 ? minutes + 50 : minutes - 10, task);
    } else if (reminder == 15) {
      notifyHelper.scheduledNotification(minutes < 15 ? hours - 1 : hours, minutes < 15 ? minutes + 45 : minutes - 15, task);
    } else
      notifyHelper.scheduledNotification(minutes < 20 ? hours - 1 : hours, minutes < 20 ? minutes + 40 : minutes - 20, task);
  }
}
