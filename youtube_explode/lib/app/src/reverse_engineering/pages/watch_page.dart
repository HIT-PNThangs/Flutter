...
class WatchPage extends YoutubePage<_InitialData> {
  ...
  int get videoLikeCount =>
      ...
      int.parse(_videoLikeExp.firstMatch(root.outerHtml)?.group(1)?.stripNonDigits().nullIfWhitespace ??
  ...
  int get videoDislikeCount =>
      ...
      int.parse(_videoDislikeExp.firstMatch(root.outerHtml)?.group(1)?.stripNonDigits().nullIfWhitespace ??
  ...
  static Future<WatchPage> get(YoutubeHttpClient httpClient, String videoId) {
    final url = Uri.parse('https://youtube.com/watch?v=$videoId&bpctr=9999999999&hl=en');
...
class _InitialData extends InitialData {
  ...
  late final String commentsContinuation = getContinuationContext()?.getT<String>('token') ?? '';
}
