...
class CommentsClient {
  ...
  static Future<CommentsClient?> get(YoutubeHttpClient httpClient, Video video) async {
    final watchPage =
        video.watchPage ?? await retry<WatchPage>(httpClient, () async => WatchPage.get(httpClient, video.id.value));
  ...
  ///
  static Future<CommentsClient?> getReplies(YoutubeHttpClient httpClient, String token) async {
...
onResponseReceivedEndpoints[1].reloadContinuationItemsCommand.continuationItems[2].commentThreadRenderer.comment.commentRenderer.contentText.runs[0].text   */
  List<JsonMap>? _getCommentRenderers() {
    ...
    return endpoint
            .get('reloadContinuationItemsCommand', 'appendContinuationItemsAction')!
...
class _Comment {
  ...
  late final String channelThumbnail =
      _commentRenderer.get('authorThumbnail')!.getList('thumbnails')!.last.getT<String>('url')!;

  late final String channelId = _commentRenderer.get('authorEndpoint')!.get('browseEndpoint')!.getT<String>('browseId')!;

  late final String text =
      _commentRenderer.get('contentText')!.getT<List<dynamic>>('runs')!.cast<Map<dynamic, dynamic>>().parseRuns();
  ...
  late final bool isHearted =
      _commentRenderer.get('actionButtons')?.get('commentActionButtonsRenderer')?.get('creatorHeart') != null;
