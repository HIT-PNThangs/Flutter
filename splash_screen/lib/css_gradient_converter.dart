...
Color _codeToColor(String code) {
  if (code.startsWith('rgba')) {
    List<String> listString =
        code.trim().replaceAll('rgba(', '').replaceAll(')', '').split(', ');
    if (listString.length < 4) {
      throw const FormatException(
        'Bad color code format for rgba',
      );
    }

    return Color.fromRGBO(int.parse(listString[0]), int.parse(listString[1]),
        int.parse(listString[2]), double.parse(listString[3]));
  }
