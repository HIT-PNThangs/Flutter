...
class TransformerPageView extends StatefulWidget {
  ...
  /// intber of children because the builder is called only for those children
  ...
  const TransformerPageView({
    ...
    required this.itemCount,
        ...
        this.duration = duration ?? const Duration(milliseconds: kDefaultTransactionDuration),
        super(key: key);
  ...
  static int getRealIndexFromRenderIndex({required bool reverse, required int index, required int itemCount, bool? loop}) {
  ...
  static PageController createPageController(
      {required bool reverse, required int index, required int itemCount, bool? loop, required double viewportFraction}) {
