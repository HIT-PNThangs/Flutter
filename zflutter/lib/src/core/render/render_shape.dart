...
class RenderZShape extends RenderZBox {
  ...
  ZVector get normalVector {
    assert(needsDirection, 'needs direction needs to be true so normal vector can be retrieved');
    debugTransformed();
  ...
  void performTransformation() {
    ...
    transformedPath = <ZPathCommand>[
      if (path.isEmpty) ZMove(0, 0, 0) else if (path.first is! ZMove) ZMove.vector(path.first.point()),
      ...path,
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (length <= 1) {
      ...
      final builder = ZPathBuilder()..renderPath(transformedPath, isClosed: isClosed);
