...
class PhotoShareBottomSheet extends StatelessWidget {
  ...
  const PhotoShareBottomSheet({Key? key, required this.scrollController}) : super(key: key);
                ...
                slivers: <Widget>[
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverContainer(
                        ...
                        sliver: SliverList(
                          delegate: SliverChildListDelegate.fixed(List<Widget>.from(actions.map(
                            (action) => Container(
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverContainer(
                        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)),
                        sliver: SliverList(
                          delegate: SliverChildListDelegate.fixed(List<Widget>.from(actions1.map(
                            (action) => Container(
                                padding: const EdgeInsets.symmetric(vertical: 16, horizontal: 16),
                                child: Text(
                                  action.title,
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverContainer(
                        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)),
                        sliver: SliverList(
                          delegate: SliverChildListDelegate.fixed(List<Widget>.from(actions2.map(
                            (action) => Container(
                                padding: const EdgeInsets.symmetric(vertical: 16, horizontal: 16),
                                child: Text(
                                  action.title,
...
extension ListUtils<T> on Iterable<T> {
  /// Creates a new list where [T] item is added
  /// between elements
  List<T> addItemInBetween(T item) {
    return withItemInBetween(item).toList();
  }

  /// Returns a new iterable where [T] item is added
  /// between the elements of the iterable
  Iterable<T> withItemInBetween(T item) sync* {
    final it = iterator;
