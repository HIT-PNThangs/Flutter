...
import '../src/modal_bottom_sheet/material_with_modal_page_route.dart';
...
class Examples {
              ...
              child: CupertinoTabView(
                onGenerateRoute: (settings) {
                  return MaterialWithModalsPageRoute(
                      settings: settings, builder: (context) => const ModalBottomSheetExample());
                },
              ),
            ),
          )
  ...
  static final Example flutterLogo = Example(
      ...
      builder: (_) => const FlutterAnimation(),
  ...
  static final Example widgetHitTest3D = Example(
      ...
      builder: (_) => const InkWell3D(),
      logoStyle: FlutterLogoColor.white);

  static final Example dice = Example(
    title: 'Dice',
    route: '/dice',
    builder: (_) => const Dices(),
  );

  static List<Example> get list => [onTheGo, iphone, flutterLogo, dice, widgetHitTest3D];
}
  ...
  static final Example simpleLine = Example(
    ...
    builder: (_) => Template(
      childrenBuilder: () => [
        ZShape(
          path: [
            const ZMove.vector(ZVector.only(x: -40)),
            ZLine.vector(const ZVector.only(x: 40)),
          ],
          stroke: 20,
  ...
  static final Example zPlain = Example(
    ...
    builder: (_) => Template(
      childrenBuilder: () => [
        ZShape(
          path: [
            const ZMove.vector(ZVector.only(x: -32, y: -40)), // start at top left
            ZLine.vector(const ZVector.only(x: 32, y: -40)), // line to top right
            ZLine.vector(const ZVector.only(x: -32, y: 40)), // line to bottom left
            ZLine.vector(const ZVector.only(x: 32, y: 40)), // line to bottom right
          ],
          closed: false,
          stroke: 20,
          color: const Color(0xff663366),
  ...
  static final Example z3d = Example(
    ...
    builder: (_) => Template(
        childrenBuilder: () => [
              ZShape(
                path: [
                  const ZMove.vector(ZVector.only(x: -32, y: -40, z: 40)), // start at top left
                  ZLine.vector(const ZVector.only(x: 32, y: -40)), // line to top right
                  ZLine.vector(const ZVector.only(x: -32, y: 40, z: 40)), // line to bottom left
                  ZLine.vector(const ZVector.only(x: 32, y: 40, z: -40)), // line to bottom right
                ],
                closed: false,
                stroke: 20,
                color: const Color(0xff663366),
        ...
        childrenBuilder: () => [
              ZShape(
                path: [
                  const ZMove.vector(ZVector.only(x: -32, y: -40)), // start at top left
                  ZLine.vector(const ZVector.only(x: 32, y: -40)), // line to top right
                  const ZMove.vector(ZVector.only(x: -32, y: 40)), // line to bottom left
                  ZLine.vector(const ZVector.only(x: 32, y: 40)), // line to bottom right
                ],
                closed: false,
                stroke: 20,
  ...
  static final Example bezier = Example(
    ...
    builder: (_) => Template(
        childrenBuilder: () => [
              ZShape(
                path: [
                  const ZMove.vector(ZVector.only(x: -60, y: -60)),
                  ZBezier([
                    const ZVector.only(x: 20, y: -60),
                    const ZVector.only(x: 20, y: 60),
                    const ZVector.only(x: 60, y: 60)
                  ]),
                ],
                closed: false,
                stroke: 20,
    ...
    builder: (_) => Template(
        childrenBuilder: () => [
              ...generateArc(
                const ZVector.only(x: -60, y: -60),
                const ZVector.only(x: 20, y: -60),
                const ZVector.only(x: 20, y: 20),
              ),
              ...generateArc(
                  const ZVector.only(x: 20, y: 20), const ZVector.only(x: 20, y: 60), const ZVector.only(x: 60, y: 60))
            ]),
  );
  static final Example openTriangle = Example(
    ...
    builder: (_) => Template(
        childrenBuilder: () => [
              ZShape(
                path: [
                  const ZMove.vector(ZVector.only(x: 0, y: -40)),
                  ZLine.vector(const ZVector.only(x: 40, y: 40)),
                  ZLine.vector(const ZVector.only(x: -40, y: 40)),
                ],
                closed: false,
                stroke: 20,
    ...
    builder: (_) => Template(
        childrenBuilder: () => [
              ZShape(
                ...
                stroke: 20,
                color: const Color(0xff663366),
                backfaceColor: const Color(0xff000000),
              ),
            ]),
  );
    ...
    builder: (_) => Template(
        childrenBuilder: () => [
              ZPositioned(
                ...
                translate: const ZVector.only(x: 0),
