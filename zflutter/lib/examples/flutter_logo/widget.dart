...
class _FlutterAnimationState extends State<FlutterAnimation> with TickerProviderStateMixin {
  ...
  update() {
    Future.delayed(const Duration(seconds: 2), () {
      if (mounted) animationController.forward(from: 0).whenComplete(() => update());
    });
  }
        ...
        builder: (context, controller) {
          return AnimatedBuilder(
            ...
            builder: (context, _) {
              ...
              final dash = dashAnimations.fold<double>(0, (previousValue, element) => previousValue + element.value);

              return ZIllustration(
                zoom: zoom ?? 1,
                children: [
                  ZPositioned(
                    rotate: ZVector.only(y: rotationAnimation, x: xrotate) + controller.rotate,
                      ...
                      children: [
                        ...
                        ZGroup(sortMode: SortMode.update, children: [
                          ZPositioned(
                            ...
                            translate: const ZVector.only(z: -50, y: 24, x: 2),
                            child: Dash(flight: dash),
                          ),
                          ZPositioned(
                            rotate: const ZVector.only(y: tau / 2 + tau / 40, x: -tau / 40),
                            translate: const ZVector.only(z: -50, y: 70, x: 40),
                            child: Dash(flight: 1 - dash),
                          ),
                          ZPositioned(
                            rotate: const ZVector.only(y: tau / 2 - tau / 40, x: -tau / 40),
                            translate: const ZVector.only(z: -50, y: -20, x: 40),
                            child: Dash(flight: dash),
                          ),
                        ]),
                        ZGroup(sortMode: SortMode.update, children: [
                          ZPositioned(
                            rotate: const ZVector.only(y: tau / 2 + tau / 40, x: -tau / 30),
                            translate: const ZVector.only(z: -50, y: -45, x: -75),
                            child: Dash(flight: 1 - dash),
                          ),
                          ZPositioned(
                            rotate: const ZVector.only(y: tau / 2 + tau / 45),
                            translate: const ZVector.only(z: -50, y: -85, x: -35),
                            child: Dash(flight: dash),
                          ),
                          ZPositioned(
                            rotate: const ZVector.only(y: tau / 2),
                            translate: const ZVector.only(z: -50, y: -130, x: 5),
                            child: Dash(flight: 1 - dash),
                          ),
                          ZPositioned(
                            rotate: const ZVector.only(y: tau / 2 - tau / 40, x: tau / 40),
                            translate: const ZVector.only(z: -50, y: -170, x: 50),
                            child: Dash(flight: dash),
                          )
                        ])
                      ],
