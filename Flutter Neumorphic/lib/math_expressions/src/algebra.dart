...
class Point3 extends Vector3 {
  ...
  Point3 lerp(Point3 p2, num coeff) => Point3(
      x * coeff + p2.x * (1 - coeff),
      y * coeff + p2.y * (1 - coeff),
      z * coeff + p2.z * (1 - coeff));
  // TODO 3d lerp?

  /// Transforms the point to its homogeneous vector4 representation.
  /// The w component is set to 1.
  Vector4 toVec4() => Vector4(x, y, z, 1.0);
...
class Interval implements Comparable<Interval> {
  ...
  /// to work properly.
  Interval(this.min, this.max) : _emptySet = false;
  ...
  Interval operator +(Interval i) {
    if (isEmpty() || i.isEmpty()) {
      ...
      return Interval(min + i.min, max + i.max);
    }
  }
  ...
  Interval operator *(Interval i) {
    if (isEmpty() || i.isEmpty()) return Interval.empty();
    final num min = _min(
    ...
    if (i.containsZero()) {
      ...
      if (!isPositive()) {
        ...
        if (i.min < i.max && i.max == 0) {
          // round down new min
          return Interval(max / i.min, double.infinity);
        }

        if (i.min < i.max && i.min == 0) {
          // round up new max
          return Interval(double.negativeInfinity, max / i.max);
        }
      }
      ...
      if (max > 0) {
        ...
        if (i.min < i.max && i.max == 0) {
          // round up new max
          return Interval(double.negativeInfinity, min / i.min);
        }

        if (i.min < i.max && i.min == 0) {
          // round down new min
          return Interval(min / i.max, double.infinity);
        }
      }
  ...
  bool operator ==(Object i) =>
      (i is Interval) && min == i.min && max == i.max;

  /// Less than operator on intervals.
  ///
  ///     [a, b] < [c, d], if a < c && b < d
  bool operator <(Interval i) => min < i.min && max < i.max;
  ...
  bool includes(Interval i) => min <= i.min && i.max <= max;
  ...
  bool contains(num element) => min <= element && element <= max;

  /// Returns true, if this interval contains zero (min <= 0 <= max).
  bool containsZero() => min <= 0 && 0 <= max;
  ...
  bool isBound() => !min.isInfinite && !max.isInfinite;

  /// Returns true, if this is the empty set.
  bool isEmpty() => _emptySet;
