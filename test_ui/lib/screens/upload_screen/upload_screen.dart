...
import '../add_audio/add_audio.dart';
import '../add_audio/bloc/add_audio_bloc.dart';
...
class _UploadPageState extends State<UploadPage> {
  ...
  Future<String> encodeVideo() async {
    ...
    await flutterFFmpeg
        .execute("-i ${widget.video} -vf scale=480:-2,setsar=1:1 -c:v libx264 -c:a copy $outPath")
          ...
          TextButton(
            onPressed: () async {
              ...
              await FirebaseFirestore.instance.collection('Videos').doc().set({
                ...
                "song_name": "Song Title ${Random().nextInt(1000)} - Artist${Random().nextInt(1000)}",
                "url": url,
                "gif": gifUrl,
              ...
              if (snapShot.connectionState == ConnectionState.done) {
                ...
                return const Center(
                  child: SizedBox(
                    ...
                    height: 70,
                    child: CircularProgressIndicator(
                        backgroundColor: Colors.grey, valueColor: AlwaysStoppedAnimation<Color>(Colors.blueAccent)),
                  ),
                );
              }
            },
          ...
          isShow
              ? Center(
                  child: FloatingActionButton(
                    ...
                    child: Icon(controller.value.isPlaying ? Icons.pause : Icons.play_arrow, color: Colors.black),
                  ),
                )
              : Container()
        ],
