...
import '../../channels/channel_link.dart';
...
class ChannelAboutPage extends YoutubePage<_InitialData> {
  ...
  ChannelAboutPage.parse(String raw) : super(parser.parse(raw), (root) => _InitialData(root));
...
class _InitialData extends InitialData {
  ...
  late final List<ChannelLink> channelLinks = content
          ...
          ?.map((e) => ChannelLink(
              ...
              extractUrl(
                  e.get('navigationEndpoint')?.get('commandMetadata')?.get('webCommandMetadata')?.getT<String>('url') ??
  ...
  late final int? viewCount = content.get('viewCountText')?.getT<String>('simpleText')?.parseInt();
  ...
  late final List<JsonMap> avatar = content.get('avatar')!.getList('thumbnails')!;

  late final String? country = content.get('country')?.getT<String>('simpleText');
