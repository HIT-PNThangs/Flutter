...
import '../flutter_shake_animated.dart';
...
class _ShakeWidgetState extends State<ShakeWidget> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(
        vsync: this, duration: widget.duration ?? widget.shakeConstant.duration);
    ...
    if (widget.shakeConstant.translate.isEmpty) {
      ...
      _translateAnimation =
          ConstantTween(widget.shakeConstant.translate.first).animate(_controller);
    } else {
    ...
    if (widget.shakeConstant.opacity.isEmpty) {
      ...
      _opacityAnimation = TweenSequence(
        List.generate(widget.shakeConstant.opacity.length - 1, (index) {
          ...
          return TweenSequenceItem(
              tween: Tween(begin: beginOpacity, end: endOpacity), weight: weight);
        }),
      ).animate(_controller);
    }
    ...
    if (widget.shakeConstant.rotate.isEmpty) {
      ...
      _rotateAnimation =
          ConstantTween(widget.shakeConstant.rotate.first * 1.0).animate(_controller);
    } else {
